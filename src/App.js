...
export default function App() {
  ...
  const submit = async (ev) => {
    try {
      ...
      const response = await fetch(`http://${process.env.REACT_APP_DOMAIN}/api/blogs`, { body: data, method: "POST" })
      ...
      <main className="container">
        <div>
          <form className="blogForm" onSubmit={submit}>
            <TextField label='Title' variant='outlined' className="blogTitle" inputRef={titleRef}></TextField>
            <Button type="button" color='primary' variant='contained' component='label' className="uploadImage">
              <ImageIcon />
          ...
          <ButtonGroup color="primary" variant='contained' className='EditorButtons'>
